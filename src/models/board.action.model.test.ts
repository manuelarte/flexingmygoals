...
test('ActionPositions check sort in constructor', () => {
  ...
  const other = [
    new BoardMoveTimestamp(new BoardPosition(0.5, 0.5), 0.5),
    new BoardMoveTimestamp(new BoardPosition(0.2, 0.2), 0.2),
  ]
...
test('ActionPositions get position for time 0.5 from 0 to 1', () => {
  const initialPosition = new BoardPosition(0, 0)
  const other = [
    new BoardMoveTimestamp(new BoardPosition(1, 1), 1),
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
...
test('ActionPositions get position for an exact other time 0.5', () => {
  const initialPosition = new BoardPosition(0, 0)
  const other = [
    new BoardMoveTimestamp(new BoardPosition(0.5, 0.6), 0.5),
    new BoardMoveTimestamp(new BoardPosition(1, 1), 1),
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
