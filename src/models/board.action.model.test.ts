...
import { BoardActorMoves, BoardMoveTimestamp, BoardPosition } from './board.action.model.ts'

test('ActionPositions check sort in constructor', () => {
  const initialPosition = BoardPosition.of(0, 0)
  const other = [
    new BoardMoveTimestamp(0.5, BoardPosition.of(0.5, 0.5)),
    new BoardMoveTimestamp(0.2, BoardPosition.of(0.2, 0.2)),
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
...
test('ActionPositions get position for time 0.5 from 0 to 1', () => {
  ...
  const other = [
    new BoardMoveTimestamp(1, new BoardPosition(1, 1)),
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
...
test('ActionPositions get position for time 0.5 from 0.5 to 1', () => {
  ...
  const other = [
    new BoardMoveTimestamp(1, BoardPosition.of(1, 1)),
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
...
test('ActionPositions get position for time only initial value', () => {
  ...
  const other: Array<BoardMoveTimestamp> = [
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
...
test('ActionPositions get position for an exact other time 0.5', () => {
  const initialPosition = BoardPosition.of(0, 0)
  const other = [
    new BoardMoveTimestamp(0.5, BoardPosition.of(0.5, 0.6)),
    new BoardMoveTimestamp(1, BoardPosition.of(1, 1)),
  ]
  const actionPositions = new BoardActorMoves(initialPosition, other)
